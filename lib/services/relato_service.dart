...
// =========================================================
Future<List<Map<String, dynamic>>> getAllRelatos() async { // NOME MUDADO
 try {
 ...
 if (response.statusCode != 200) {
  throw Exception("Erro ao buscar todos os relatos: ${response.statusCode} - ${response.data}");
 }
...
// =========================================================
Future<List<Map<String, dynamic>>> getMyRelatos() async { // NOVO MÉTODO
 try {
 final response = await _dio.get(
 ...
 if (response.statusCode != 200) {
  throw Exception("Erro ao buscar meus relatos: ${response.statusCode} - ${response.data}");
 }
...
Future<List<Map<String, dynamic>>> fetchLatestRelatos({
 ...
 if (token.isEmpty) {
  log('AVISO: Token JWT ausente para fetchLatestRelatos, a API pode negar acesso.', level: 800);
 }
 ...
 try {
  ...
  if (response.statusCode != 200) {
   throw Exception("Erro ao buscar últimos relatos: ${response.statusCode} - ${response.data}");
  }
...
Future<String?> deleteRelato(int relatoId) async {
 // Log de Debug Adicionado AQUI
  ...
  if (token.isNotEmpty) {
    ...
    log('TOKEN INÍCIO: ${token.length > 15 ? token.substring(0, 15) : token}...'); 
  } else {
